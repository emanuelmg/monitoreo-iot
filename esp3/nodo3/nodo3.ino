...
//    conteo = conteo + 1;
 ...
 WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
   ...
   // if(conteo == 50){
   //   ESP.deepSleep(1e6);
   //   Serial.println("me dormire no internet");
      //}
  }
...
void enviar(){
   ...
   float tempP = adc1 * multiplier * 100;
   ...
   strtempp = String(tempP, 2);
   strtempp.toCharArray(str_tempp, 15);
   client.publish("tpanel", str_tempp);
   Serial.print("Send T_panel ");
   Serial.println(str_tempp);
    ...
    stripanel = String(ipanel);
    stripanel.toCharArray(str_ipanel, 15);
    client.publish("/esp3/ipanel", str_ipanel);
    Serial.print("Send i_panel: ");
    Serial.println(str_ipanel);

    strrad = String(rad);
    strrad.toCharArray(str_rad, 15);
    client.publish("rad1", str_rad);
    Serial.print("Send rad: ");
    Serial.println(str_rad);
    delay(2000);  
  }
